...
const EmployeeSalaryReport = () => {
    ...
    const fetchEmployeeReport = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8070/salary/employee/${employeeId}/${selectedYear}/${selectedMonth}`
        );
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(13);
    doc.text(
      `Monthly Salary: ${formatCurrency(reportData.monthlySalary)} LKR`,
      25,
      y + 3
    );
    y += lineSpacing + 4;
    ...
    doc.save(
      `Salary_Report_${reportData.employeeId}_${selectedMonth}_${selectedYear}.pdf`
    );
  };
  ...
  if (loading) {
    return (
      <div className="SRa-container">
        <div className="SRa-loading">Loading salary report...</div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="SRa-container">
        <div className="SRa-error">{error}</div>
        <button className="SRa-back-button" onClick={() => navigate(-1)}>
          &larr; Go Back
        </button>
      </div>
    );
  ...
  return (
    <div className="SRa-container">
      ...
      <h2 className="SRa-heading">Employee Salary Report</h2>

      <div className="SRa-month-selector">
      ...
      <div className="SRa-card">
        <div className="SRa-employee-info">
          <h3>{reportData.name}</h3>
          <p>
            <strong>Employee ID:</strong> {reportData.employeeId}
          </p>
          <p>
        ...
        <div className="SRa-salary-details">
          <div className="SRa-detail-row">
          ...
          <div className="SRa-detail-row SRa-total-salary">
      ...
      <button className="SRa-download-button" onClick={downloadPDF}>
        Download PDF
      </button>
    </div>
  );
